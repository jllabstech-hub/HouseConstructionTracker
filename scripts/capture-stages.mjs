...
async function main() {
  ...
  // 1. Desktop Stages View (1280x900)
  console.log("Capturing Desktop Construction Stages View...");
  ...
  await desktopPage.goto("http://localhost:7000/stages", { waitUntil: "networkidle" });
  await desktopPage.waitForTimeout(2000);

  const desktopPath = path.join(artifactDir, "stages_redesign_desktop.png");
  await desktopPage.screenshot({ path: desktopPath, fullPage: true });
  console.log("Desktop Stages screenshot saved:", desktopPath);

  // 2. Mobile Stages View (390x844)
  console.log("Capturing Mobile Construction Stages View...");
  ...
  await mobilePage.goto("http://localhost:7000/stages", { waitUntil: "networkidle" });
  await mobilePage.waitForTimeout(2000);

  const mobilePath = path.join(artifactDir, "stages_redesign_mobile.png");
  await mobilePage.screenshot({ path: mobilePath, fullPage: true });
  console.log("Mobile Stages screenshot saved:", mobilePath);
