...
async function main() {
  ...
  // 1. Desktop Documents View (1280x900)
  console.log("Capturing Desktop Documents View...");
  ...
  await desktopPage.goto("http://localhost:7000/documents", { waitUntil: "networkidle" });
  await desktopPage.waitForTimeout(2000);

  const desktopPath = path.join(artifactDir, "documents_redesign_desktop.png");
  await desktopPage.screenshot({ path: desktopPath, fullPage: true });
  console.log("Desktop Documents screenshot saved:", desktopPath);

  // 2. Mobile Documents View (390x844)
  console.log("Capturing Mobile Documents View...");
  ...
  await mobilePage.goto("http://localhost:7000/documents", { waitUntil: "networkidle" });
  await mobilePage.waitForTimeout(2000);

  const mobilePath = path.join(artifactDir, "documents_redesign_mobile.png");
  await mobilePage.screenshot({ path: mobilePath, fullPage: true });
  console.log("Mobile Documents screenshot saved:", mobilePath);
