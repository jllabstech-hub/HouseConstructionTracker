...
async function main() {
  ...
  // 1. Desktop Budget View (1280x900)
  console.log("Capturing Desktop Budget View...");
  ...
  await desktopPage.goto("http://localhost:7000/budget", { waitUntil: "networkidle" });
  await desktopPage.waitForTimeout(2000);

  const desktopPath = path.join(artifactDir, "budget_redesign_desktop.png");
  await desktopPage.screenshot({ path: desktopPath, fullPage: true });
  console.log("Desktop Budget screenshot saved:", desktopPath);

  // 2. Mobile Budget View (390x844)
  console.log("Capturing Mobile Budget View...");
  ...
  await mobilePage.goto("http://localhost:7000/budget", { waitUntil: "networkidle" });
  await mobilePage.waitForTimeout(2000);

  const mobilePath = path.join(artifactDir, "budget_redesign_mobile.png");
  await mobilePage.screenshot({ path: mobilePath, fullPage: true });
  console.log("Mobile Budget screenshot saved:", mobilePath);
