      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-lg font-bold text-ink-900">
            {language === "te" ? "కేటగిరీ వారీ బడ్జెట్ పరిమితులు" : "Specific Item & Trade Limits"}
          </h2>
          <p className="text-xs text-ink-500">
            {language === "te" ? "సిమెంట్, ఇనుము, ఇసుక, టైల్స్, ప్లంబింగ్ తదితర ఖర్చుల పరిమితులు" : "Limits set on Cement, Steel, Sand, Tiles, Plumbing, etc."}
          </p>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            type="text"
            placeholder={language === "te" ? "కేటగిరీలను వెతకండి..." : "Search category limits..."}
            value={query}
            onChange={(e) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="bg-paper-100 text-[11px] font-bold uppercase tracking-wider text-ink-500 border-b border-paper-200">
            <tr>
              <th className="px-4 py-3">{language === "te" ? "కేటగిరీ / అంశం" : "Category / Item"}</th>
              <th className="px-4 py-3">{t.budget.planned}</th>
              <th className="px-4 py-3">{t.budget.actual}</th>
