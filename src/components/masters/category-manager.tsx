...
type Kind = "material" | "labour" | "other" | "stage";

export function CategoryManager({ projectId, materials, labours, services, stages }: { projectId: string; materials: Category[]; labours: Category[]; services: Category[]; stages: Category[] }) {
  ...
  const list = kind === "material" ? materials : kind === "labour" ? labours : kind === "other" ? services : stages;
  ...
  const save = () => start(async () => {
    ...
    const result = editing
      ? kind === "material" ? await updateMaterialCategory(editing.id, input) : kind === "labour" ? await updateLabourCategory(editing.id, input) : kind === "other" ? await updateServiceCategory(editing.id, input) : await updateConstructionStageName(projectId, editing.id, name)
      : kind === "material" ? await createMaterialCategory(input) : kind === "labour" ? await createLabourCategory(input) : kind === "other" ? await createServiceCategory(input) : await createConstructionStageAction({ projectId, name });
  ...
  return <div className="w-full space-y-5 pb-8">
    <header className="flex flex-col gap-2 border-b border-paper-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-wrap rounded-xl border border-paper-300 bg-white p-1 text-xs font-bold">
        {(["material", "labour", "other", "stage"] as Kind[]).map((value) => <button key={value} type="button" onClick={() => { setKind(value); reset(); }} className={`rounded-lg px-3 py-2 capitalize ${kind === value ? "bg-clay-600 text-white" : "text-ink-600"}`}>{value === "stage" ? "Stages" : value}</button>)}
      </div>
    </header>
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="rounded-2xl border border-paper-200 bg-white p-4 sm:p-5">
        ...
        <div className="mt-4 divide-y divide-paper-100">
          {filtered.length ? filtered.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-ink-900">{item.name}</p>{kind !== "stage" && kind !== "other" && item.groupName && <p className="text-xs text-ink-500">{item.groupName}</p>}</div><div className="flex shrink-0 gap-1"><button type="button" onClick={() => { setEditing(item); setName(item.name); setGroupName(item.groupName ?? ""); setError(null); }} className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-xs font-semibold text-ink-600 hover:bg-paper-100"><Edit3 className="h-3.5 w-3.5" />Edit</button>{kind === "labour" && <button type="button" onClick={() => { if (window.confirm(`Move “${item.name}” and its expenses to Other?`)) start(async () => { const result = await moveLabourCategoryToOther(item.id); if ("error" in result && result.error) setError(result.error); else window.location.reload(); }); }} className="rounded-lg px-2 py-2 text-xs font-semibold text-clay-700 hover:bg-clay-50">Move to Other</button>}<button type="button" onClick={() => setDeleting(item)} className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-xs font-semibold text-red-700 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" />Remove</button></div></div>) : <p className="py-10 text-center text-sm text-ink-500">No {kind === "stage" ? "stages" : `${kind} categories`} yet. Add your first one.</p>}
        </div>
      </div>
    ...
    <ConfirmDialog open={Boolean(deleting)} onClose={() => setDeleting(null)} onConfirm={() => { if (!deleting) return; start(async () => { const result = kind === "material" ? await deleteMaterialCategory(deleting.id) : kind === "labour" ? await deleteLabourCategory(deleting.id) : kind === "other" ? await deleteServiceCategory(deleting.id) : await deleteConstructionStageAction(projectId, deleting.id); if ("error" in result && result.error) setError(result.error); else window.location.reload(); setDeleting(null); }); }} title={`Delete ${kind === "stage" ? "stage" : "category"}?`} description={kind === "stage" ? `Expenses linked to “${deleting?.name ?? ""}” will remain but be unassigned.` : `Existing expenses will keep their amounts but no longer be linked to “${deleting?.name ?? ""}”.`} confirmText={pending ? "Deleting…" : "Delete"} variant="danger" />
  </div>;
}
